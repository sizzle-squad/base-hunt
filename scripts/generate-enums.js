...
const fileContent = fs.readFileSync('src/utils/database.types.ts', 'utf8');
  ...
  return Object.entries(enums)
    .map(([enumName, enumValues]) => {
      const enumMembers = enumValues
        .map((value) => `${replace_special_chars(value)} = "${value}"`)
        .join(',\n  ');
...
fs.writeFileSync('./src/utils/database.enums.ts', enumStrings);
