...
async function getTxDetails(txHash: string, address: string) {
    ...
    try {
        ...
        if (!response.ok) {
            console.error('Coinbase API TransactionDetailsResponse fetch failed:', response.status);
            return null;
        }
        ...
        console.error('Coinbase API TransactionDetailsResponse fetch failed:', error);
        return null;
    }
}

export async function POST(request: NextRequest) {
    const authResponse = await authenticate(request, async () => NextResponse.next());
    ...
    if (!userAddress || !txHash) {
        return new Response(
            `Missing parameters: userAddress: ${userAddress}, txHash: ${txHash}`,
            {
                status: 400,
            }
        );
    ...
    let assetAddress = txDetails?.transaction?.transfers[txDetails?.transaction?.transfers.length - 1]?.assetAddress;
    if (!assetAddress) {
        console.error('Transaction not found:', txHash);
        return new Response(
            `Transfers not found for txHash: ${txHash}`,
            {
                status: 400,
            }
        );
    ...
    const { error: findOrCreateCommunityError } = await supabase.rpc('find_or_create_community', {
        _asset_address: assetAddress,
        _symbol: addressMeta.token?.symbol,
        _logo: addressMeta.token?.logo?.url,
    });
    if (findOrCreateCommunityError) {
        console.error('Error finding or creating community:', findOrCreateCommunityError);
